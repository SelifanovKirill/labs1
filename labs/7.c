...
int main() {
    ...
    for (;;) { /*start*/
        ...
        for (x = sx; x <= ex; x += step) {
            ...
            if (fabs( sin(20 *(a * a)+ 9 * a * x - 20 *(x * x) - 3.14/2)) >= 0.0001) {
                result.F[index] = (-1) / ( sin( 20 *(a * a) + 9 * a * x - 20 * (x * x) - 3.14/2));
            } else {
                printf("При X = %lf, A = %lf, функцию F вычислить невозможно.\n", x, a);
            }

            if ( fabs(a * a - 6 * a * x + 5 * (x * x) + 1) <= 1) {
                result.Y[index] = acos(a * a - 6 * a * x + 5 * (x * x) + 1);
            } else {
        ...
        index = (int) sx;
        if ((file1 = fopen("/home/user/Desktop/test.txt", "w")) == NULL){
            printf("Ошибка(1)\n");
        } else {
            printf("Успешно(1)\n");
        }
        ...
        if ((file2 = fopen("/home/user/Desktop/test2.txt", "r")) == NULL){
            printf("Ошибка(2)\n");
        } else {
            printf("Успешно(2)\n");
        }
